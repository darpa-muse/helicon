...
def check_uuid(uuid):
    ...
    if uuid != uuid.lower():
        raise ValueError('uuid is not lowercase'.format(uuid))
...
def check_md5_hash(data, md5):
    h = hashlib.md5(data).hexdigest()
    if h != md5:
        raise ValueError('hash of data not equal to md5')
...
def path_filter(path):
    """
    Return True if path needs to be filtered out (not source code)
    """
    basename = os.path.basename(path)
    if '.' not in basename:
        return True
    ext = basename.split('.')[-1]
    return ext.lower() not in EXTS
