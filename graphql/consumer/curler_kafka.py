...
def extract(id,project):
    ...
    os.system("cd "+project_location+ " && unzip -o "+file_name+ " && rm "+file_name)
...
def send_to_kafka(id,code_location):
    producer = KafkaProducer(bootstrap_servers=[kafka_string], value_serializer=lambda m: json.dumps(m).encode('ascii'))
...
consumer = KafkaConsumer('graphql_projects',
                         group_id='graphql-1',
