...
# set default values
timeout=1000   # time to run doxygen process before bailing 
maxfiles=50000
san="false" # determine if we need to untar src code from san
count=0
countdone=0
one="false"
logger="doxygenTimeouts07.txt"
...
do
  ...
  then
    ...
    if [[ ! -z "$found" ]]; then
       echo "determined this to be timeout project, skipping"
       continue
    fi 
    ...
    if [ -f $project/index.json ]; then
       ...
       then
          ...
          echo ""
             ...
             echo "Timeout reached, adding uid to redis set:doxygen-timeouts"
             redis-cli -n 0 SADD "set:doxygen-timeouts" "$uid"
             echo $uid >> $logger
          fi
